...
def get_files_info(clnt):
    fmt_str = 'Q' #unsigned long long type
    headsize = struct.calcsize(fmt_str) #equal to 8
    head_data=clnt.recv(headsize)
    infos_len = struct.unpack(fmt_str,head_data)[0]#get the size of info
    data=recv_unit_data(clnt,infos_len)
    return pickle.loads(data)#return a list of info
